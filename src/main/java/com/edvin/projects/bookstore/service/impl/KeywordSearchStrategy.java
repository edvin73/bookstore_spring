...
public class KeywordSearchStrategy implements SearchStrategy {
	...
	public List<LibraryItem> search(List<LibraryItem> items, String query) {
		// Implement the logic to search for items based on keywords
		// This could involve filtering items that contain the query in their keywords
		return items.stream()
				.filter(item -> item.getKeywords().stream()
						.anyMatch(keyword -> keyword.getKeyword().toLowerCase().equals(query.toLowerCase())))
